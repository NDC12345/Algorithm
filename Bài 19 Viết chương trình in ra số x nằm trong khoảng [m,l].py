...
def isPrime(n, k):
    ...
    for i in range(k):
        if(millerTest(r, n) == False):
            return False
    return True
def find_x(a, b, c, m, l):
    i = m
    k = 4
    result = []
    while i <= l:
        if (isPrime((a*(i**2)+b*i+c), k) == True):
            result.append(i)
        i = i + 1
    return result
...
l = int(input("Nhập số nguyên dương l:"))
print("Số nguyên dương x nằm trong khoảng [%d, %d] thỏa mãn A*x^2 +B*i + C là số nguyên tố:" %(m,l), ", ".join(str(i) for i in find_x(a,b,c,m,l)))